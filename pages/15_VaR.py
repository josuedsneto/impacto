...
if st.button('Calcular'):
    ...
    def _br(v, dec=2): return f"{abs(v):,.{dec}f}".replace(",","X").replace(".","," ).replace("X",".")
    def _pct(v): return f"{v*100:.2f}".replace(".", ",") + "%"
    col1.metric("VaR (Perda Máx.)", _br(VaR_EWMA))
    col2.metric("Preço em Risco",   _br(price_at_risk))
    col3.metric("Média Retornos Diários", _pct(mean_returns))
    col4.metric("Vol. EWMA Diária", _pct(ewma_vol_final))
    col5.metric("Z-Score", f"{z_score:.4f}".replace(".", ","))
