...
@st.cache_data(ttl=3600)
def _ajustar_arima_dolar(close_tuple: tuple, p: int, d: int, q: int, dias_futuro: int) -> list:
...
def arima_previsao_dolar(df, dias_futuro, p=5, d=1, q=0):
    with st.spinner("Ajustando modelo ARIMA..."):
        forecast_list = _ajustar_arima_dolar(tuple(df['Close'].values), p, d, q, dias_futuro)
